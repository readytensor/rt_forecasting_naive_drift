...
class Forecaster:
    ...
    def _fit_on_series(self, history: pd.DataFrame, data_schema: ForecastingSchema):
        ...
        total_drift = time_series[-1] - time_series[0]
        average_drift = total_drift / (len(time_series) - 1)
        last_value = time_series[-1]
        return last_value, average_drift
    ...
    def _predict_on_series(self, key_and_future_df):
        ...
        if self.drifts.get(key) is not None:
            value, drift = self.drifts.get(key)
            forecasts = [value + (i + 1) * drift for i in range(len(future_df))]
